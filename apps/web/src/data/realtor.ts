/** Sample profile for the demo. Figures/credentials are illustrative (not a real license record). */
export const REALTOR = {
  name: "Nilyan Herrera",
  monogram: "NH",
  tagline: "Real Estate · Miami",
  email: "hola@nilyanherrera.com",
  phone: "+1 (305) 555 0148",
  office: "2000 Ponce de Leon Blvd, Coral Gables, FL",
  // Nilyan's real Instagram (realtor account) — the one social surface we link site-wide.
  instagram: "https://www.instagram.com/nh.realestateadvisor/",
  instagramHandle: "@nh.realestateadvisor",
  license: "FL License #SL3492210",
  reviews: 98,
  rating: 5,
  copyrightYear: 2026,
  // Nilyan's real portrait (served from apps/web/public/images).
  photo: "/images/nilyan-herrera.jpg",
  stats: [{ value: "12" }, { value: "240+" }, { value: "$480M" }],
} as const;
